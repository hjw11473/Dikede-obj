  ...
  <div class="login-container app">
    ...
    <el-form ref="loginForm" :model="loginForm" :rules="loginRules" class="login-form" auto-complete="on" label-position="left">
      ...
      <div style="display:flex">
        <!-- item每一项 -->
        <el-form-item style="flex:3"> <!-- <span class="svg-container el-icon-user-solid" /> -->
          <span
...
export default {
  ...
  methods: {
    massg() {
      this.$alert('用户名密码或验证码错误，请重新输入', '标题名称', {
    ...
    async handleLogin() {
      try {
        ...
        if (data.success === false) {
          // this.massg()
          this.$message.error('用户名密码或验证码错误，请重新输入')
          this.Verifi()
    ...
    async Verifi() {
      try {
        const { data } = await VerificationCodeAPI(this.random)
